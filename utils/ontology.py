"""
This file was inspired by Ruoxi Shi's implementation of the SLU projection tagging task.
"""
import json
import os
import collections
...
class OntologyProjection:
    ...
    def projection(self, slot, val):
        ...
        for v in set.union(*(inv_dict[ch] for ch in val)):
            dis = levenshteinDistance(v, val), abs(len(v) - len(val))
            if dis < min_dis:
                min_dis = dis
                mini = v
        if min_dis >= (len(val) * 0.45, 256):
            return None
        return mini
