...
if not args.testing:
    ...
    optimizers = [set_optimizer(models[i], args) for i in range(N_ensemble)]
    nsamples, best_result = len(train_dataset), {"dev_acc": 0.0, "dev_f1": 0.0}
    train_index, step_size = np.arange(nsamples), args.batch_size * N_ensemble
    ...
    for i in range(args.max_epoch):
        ...
        print(
            ...
            % (
                ...
                dev_acc,
                dev_fscore["precision"],
                dev_fscore["recall"],
                dev_fscore["fscore"],
            )
        )
        if dev_acc > best_result["dev_acc"]:
            best_result["dev_loss"], best_result["dev_acc"], best_result["dev_f1"], best_result["iter"] = (
                dev_loss,
                dev_acc,
                dev_fscore,
                i,
    ...
    print(
        "FINAL BEST RESULT: \tEpoch: %d\tDev loss: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)"
        % (
            best_result["iter"],
            best_result["dev_loss"],
            best_result["dev_acc"],
            best_result["dev_f1"]["precision"],
            best_result["dev_f1"]["recall"],
            best_result["dev_f1"]["fscore"],
        )
    )
else:
    start_time = time.time()
    ...
    predict()
    print(
        "Evaluation costs %.2fs ; Dev loss: %.4f\tDev acc: %.2f\tDev fscore(p/r/f): (%.2f/%.2f/%.2f)"
        % (
            time.time() - start_time,
            dev_loss,
            dev_acc,
