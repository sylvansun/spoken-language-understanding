...
class SLUTagging(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=config.embed_size, nhead=8)
    ...
    def forward(self, batch):
        ...
        encoding = self.tokenizer(utt, padding=True, truncation=True, max_length=max(lengths))
        _input_ids = torch.tensor(encoding["input_ids"]).to(device=self.device, dtype=torch.long)
        attention_mask = torch.tensor(encoding["attention_mask"]).to(device=self.device, dtype=torch.long)
        out = self.bert(input_ids=_input_ids, attention_mask=attention_mask)

        # embed = self.word_embed(input_ids)  # (tensor) bs * S * 768
        # embed = self.transformer_encoder(embed) + embed
        ...
        # rnn_out += embed + out.last_hidden_state
        rnn_out = out.last_hidden_state
